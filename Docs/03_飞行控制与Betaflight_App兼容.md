...
三轴正方向均按右手定则：Roll右倾为正、Pitch抬头为正、Yaw从上方看顺时针为正。
ICM42688P 的板级方向基线为 `CW90_DEG`，v0.2.0已经完成加速度六向和Gyro三轴
实物确认，驱动之后不得为姿态显示再次换轴。
...
四元数顺序为 `[w,x,y,z]`，表示Body→NED。水平正面朝上时，机体Z轴朝下，
加速度计比力约为 `[0,0,-g]`；姿态算法使用 `-accel/|accel|` 作为向下重力方向。
...
- 校准输入为SI换算和 `CW90` 之后的机体系角速度。
- 250个连续静止预热样本和2000个Welford均值/方差样本。
- 单轴5 deg/s瞬时门限、0.8 g～1.2 g加速度模长门限。
- 单轴窗口标准差不超过0.5 deg/s时进入READY。
- 运动、无效值或方差超限会丢弃窗口并重新等待静止。
- IMU离线和恢复后强制重新校准；零偏不写入Flash。

校准未完成时设置 `APP_ARMING_INHIBIT_GYRO_NOT_CALIBRATED`，禁止后续ARM状态机
放行。READY后禁止在飞行中继续学习零偏。详细设计和实物验收分别见文档 `11`、`12`。

### 3.2 Accelerometer 校准

当前v0.5.0已实现水平单面校准：

- 输入为SI换算和 `CW90` 后、尚未扣除加速度偏置的机体系数据。
- 250个连续预热样本和2000个Welford统计样本。
- 使用角速度、加速度模长、水平分量、窗口方差和最终偏置多重门限。
- 明显运动或倾斜会清空窗口，不能把水平重力分量吸收到偏置。
- 新偏置必须先通过Sector 6/7参数槽提交和读回验证，之后才成为READY。
- 标准 `MSP_ACC_CALIBRATION`（205）可从Betaflight App触发。
- 重启从参数槽加载偏置；损坏、未校准或保存失败保持解锁抑制。

本版本只计算三轴零偏，不计算比例和非正交误差。六面校准以后单独立项。设计和
验收分别见文档 `13`、`14`。
...
- 只使用校准READY、时间有效和滤波READY的IMU样本。
- `Kp=2.0`、`Ki=0.05`，积分单轴限制为0.25 rad/s。
- Accel模长0.8～1.2 g时参与重力反馈，超界时只作Gyro积分。
- 首个可信样本由重力播种Roll/Pitch，Yaw置0。
- 输入或数值异常立即撤销姿态READY并重置；恢复后重新播种。
- 内部Euler范围为Roll `[-180,180]`、Pitch `[-90,90]`、Yaw `[0,360)`。
- `MSP_ATTITUDE`依次发送Roll/Pitch 0.1°和Yaw 1°；姿态无效时返回0。

详细坐标、公式、诊断和测试向量见文档 `17`，交付与实物验收见文档 `18`。
