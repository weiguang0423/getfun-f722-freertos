...
| 资源 | 引脚 | Timer 映射 | 状态 |
|---|---|---|---|
| Motor 1 | PA15 | TIM2_CH1 / AF1 | 已配置，待无桨实测 |
| Motor 2 | PA10 | TIM1_CH3 / AF1 | 已配置，待无桨实测 |
| Motor 3 | PA9 | TIM1_CH2 / AF1 | 已配置，待无桨实测 |
| Motor 4 | PA8 | TIM1_CH1 / AF1 | 已配置，待无桨实测 |
...
2. ICM42688P 的实际机体姿态方向。
3. Motor 1～4 的实体位置、编号、DShot 输出和旋转方向。
4. PC0 VBAT 和 PC1 Current 的实际标定值。
5. MAX7456 的模拟摄像头和图传画面。
6. PB0、PC14、PC15、PC4、PB2 和 PB1 的实际电气行为。
7. ST-LINK/SWD 的长期调试与 Flash 读回。
8. Option Bytes 归档。
9. 原厂 4.5.1 固件实际回写演练。
10. 当前 MSP 1.48 运行配置的完整 `diff all` 归档。
