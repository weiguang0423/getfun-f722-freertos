...
原厂 HEX 已通过格式和地址完整性校验。2026-08-13，开发者确认原厂4.5.1固件实际
回写、启动、USB连接以及再次恢复自研固件的完整演练通过。
...
6. ST-LINK/SWD 的长期调试与 Flash 读回。（已确认，2026-08-13）
7. Option Bytes 归档。（已确认，2026-08-13）
8. 原厂 4.5.1 固件实际回写演练。（已确认，2026-08-13）
9. 当前 MSP 1.48 运行配置的完整 `diff all` 归档。

每完成一项，应把测试结果更新到本文件，并将状态从“待确认”改为“已确认”。

S1.10～S1.12由开发者于2026-08-13确认验收通过：ST-LINK/SWD可用于调试和Flash
读写核对，Option Bytes已读取归档并核对关键位，原厂4.5.1可回写启动并连接USB，随后
可恢复当前自研固件。仓库未保存本次工具原始导出值，因此本文不补写未经提供的序列号、
Option Bytes数值或Flash读回哈希。
