...
38_S7.3_RKNN转换与板端NPU推理验收记录.md
39_S7.4_摄像头实时手部跟踪开发与验收.md
40_S7.5_手势集合与时序状态机开发验收.md
41_S7.6_手势到虚拟RC映射与发送链路开发验收.md
```
...
- `38`：S7.3 RKNN转换、FP16/INT8板端误差比较、性能和交付模型选择记录。
- `39`：S7.4 V4L2最新帧实时链、逐帧延迟/丢帧/失效契约和5分钟板端验收关口。
- `40`：S7.5四类手势、分类置信度、五态时序状态机和真人重复验收方法。
- `41`：S7.6保守虚拟RC映射、限幅限速、CRC帧、串口发送边界和通信实物关口。
