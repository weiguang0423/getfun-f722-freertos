...
- ARM64 程序：`Linux/RK3568/s7_3/s7_3_board_infer`（本地生成，不进入Git）。
- 主机侧证据与冻结验收范围：`Linux/RK3568/s7_3/evidence/host_preparation.json`。
...
python Linux/RK3568/s7_3/compare_results.py --pc s7_3_pc_fixed/results.jsonl --board fp16.jsonl --output fp16_compare.json
```

INT8 使用同一命令替换模型。开发者已免除 S7.3 的 30 分钟稳定性验收；`board_stability.sh`
仅保留为可选诊断工具，不作为完成关口。2026-08-14 板端全量比较选择 FP16，INT8 因误差超限拒绝。
